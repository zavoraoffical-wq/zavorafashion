...
async function db() {
  ...
  if (!uri) {
    ...
    throw new Error('Missing database connection. Add MONGODB_URI or Supabase env variables.');
  }
...
function docTable() {
  return process.env.SUPABASE_DOCUMENTS_TABLE || 'app_documents';
}
...
class SupabaseCollection {
  ...
  async rows() {
    ...
    if (!response.ok) throw new Error(data?.message || `Supabase table ${docTable()} is missing`);
    return data.map((row) => ({
  ...
  async updateOne(filter = {}, update = {}, options = {}) {
    ...
    delete next._rowId;
    const response = await fetch(this.endpoint(`?id=eq.${encodeURIComponent(existing._rowId)}`), {
      method: 'PATCH',
      headers: supabaseHeaders(),
      body: JSON.stringify({
    ...
    if (!response.ok) {
      const data = await response.json().catch(() => ({}));
      throw new Error(data?.message || `Could not update ${this.name}`);
    }
  ...
  async deleteOne(filter = {}) {
    ...
    if (!response.ok) throw new Error(`Could not delete ${this.name}`);
