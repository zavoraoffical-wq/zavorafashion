...
function jwtSecret() {
  return envValue('AUTH_JWT_SECRET', 'JWT_SECRET') || 'zavora_jwt_secret_key_2026';
}

function mongoUri() {
  return envValue('MONGODB_URI', 'MONGO_URI') || 'mongodb+srv://zavora_app:ZavoraPass2026@cluster0.m5ehvyp.mongodb.net/zavora_fashion?retryWrites=true&w=majority&appName=Cluster0';
}
...
class PostgresCollection {
  ...
  find(filter = {}) {
    ...
    const state = { sort: {}, limit: 0, skip: 0 };
    return {
      sort(sort) {
        state.sort = sort || {};
        return this;
      ...
      skip(skip) {
        state.skip = Math.max(Number(skip || 0), 0);
        return this;
      },
      limit(limit) {
  ...
  async clear() {
    ...
    const table = postgresTableName();
    const result = await postgresPool().query(
      `delete from public.${table} where collection = $1`,
      [this.name]
    );
...
class SupabaseCollection {
  ...
  async insertOne(doc = {}) {
    ...
    if (!response.ok) {
      const text = await response.text().catch(() => '');
      if (/email.*schema cache|email.*column|PGRST204/i.test(text)) {
        ...
        return this.parseSupabaseResponse({ ok: false, status: response.status, text: async () => text }, `Could not insert ${this.name}`);
      }
    }
  ...
  async updateOne(filter = {}, update = {}, options = {}) {
    ...
    const next = this.applyUpdate(existing, update);
    delete next._rowId;
    let response = await fetch(this.endpoint(`?collection=eq.${encodeURIComponent(this.name)}&doc_id=eq.${encodeURIComponent(existing._id)}`), {
      method: 'PATCH',
      headers: supabaseHeaders(),
      body: JSON.stringify({
    ...
    if (!response.ok) {
      const text = await response.text().catch(() => '');
      if (/email.*schema cache|email.*column|PGRST204/i.test(text)) {
        response = await fetch(this.endpoint(`?collection=eq.${encodeURIComponent(this.name)}&doc_id=eq.${encodeURIComponent(existing._id)}`), {
          method: 'PATCH',
          headers: supabaseHeaders(),
          body: JSON.stringify({ payload: next })
        });
      } else {
        return this.parseSupabaseResponse({ ok: false, status: response.status, text: async () => text }, `Could not update ${this.name}`);
      }
    }
    if (!response.ok) {
  ...
  async clear() {
    const response = await fetch(this.endpoint(`?collection=eq.${encodeURIComponent(this.name)}`), {
      method: 'DELETE',
      headers: supabaseHeaders({ Prefer: 'return=minimal' })
    });
    if (!response.ok) await this.parseSupabaseResponse(response, `Could not clear ${this.name}`);
    return { deletedCount: 0, cleared: true };
  }
}
