...
class NormalizationEngine {
  ...
  static normalize(raw, index = 0, requestedGender = '') {
    ...
    const primaryVariant = allVariants.find(v => {
      const c = String(v?.color || v?.color_name || v?.name || '').toLowerCase();
      return c.includes(primaryColor);
    });

    const primaryVariantImg = primaryVariant?.files?.[0]?.preview_url
      || primaryVariant?.preview_url
      || primaryVariant?.image
      || primaryVariant?.image_url;

    const rawImg = primaryVariantImg
      || raw?.thumbnail_url || raw?.image || raw?.image_url
...
class ProductRepository {
  ...
  static async bulkUpsert(products = []) {
    ...
    }).filter(op => op.updateOne.filter.printfulId);

    if (typeof col.bulkWrite !== 'function') {
      let upserted = 0;
      let modified = 0;
      for (const op of ops) {
        const res = await col.updateOne(op.updateOne.filter, op.updateOne.update, { upsert: true });
        if (res?.upsertedId || res?.upsertedCount) upserted += 1;
        else if (res?.modifiedCount || res?.matchedCount) modified += 1;
      }
      return { saved: true, upserted, modified, count: products.length, total: products.length, adapter: 'updateOne' };
    }
  ...
  static async countSummary() {
    const isPublishedProduct = (product = {}) => {
      const payload = product.payload || product;
      const status = String(payload.status || product.status || '').toLowerCase();
      return payload.published === true || product.published === true || status === 'published' || status === 'active';
    };
    const isDraftProduct = (product = {}) => {
      const payload = product.payload || product;
      const status = String(payload.status || product.status || '').toLowerCase();
      return payload.published === false || product.published === false || status === 'draft' || status === 'staged';
    };

    const col = await this.getCollection();
    if (!col) {
      const list = Array.from(memoryProductStore.values());
      const published = list.filter(isPublishedProduct).length;
      const draft = list.filter(isDraftProduct).length;
      return {
        provider: 'memory',
        total: list.length,
        published,
    ...
    const publishedFilter = {
      $or: [
        { 'payload.published': true },
        { published: true },
        { 'payload.status': { $in: ['published', 'active'] } },
    ...
    const draftFilter = {
      $or: [
        ...
        { published: false },
        { 'payload.status': { $in: ['draft', 'staged'] } },
        { status: { $in: ['draft', 'staged'] } }
      ]
    };
    ...
    const [total, published, draft, archived, deleted] = await Promise.all([
      ...
      col.countDocuments(draftFilter),
      col.countDocuments({ $or: [{ 'payload.status': 'archived' }, { status: 'archived' }] }),
      col.countDocuments({ $or: [{ 'payload.status': 'deleted' }, { status: 'deleted' }] })
    ]);
