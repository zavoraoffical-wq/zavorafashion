...
function getHomeProducts() {
  ...
  const livePrintful = (state.printfulProducts || []).filter(p => p && (p.status === 'published' || p.published !== false));
  const rawList = [...adminProducts, ...livePrintful];
  return uniqueHomeProducts(rawList).map((product) => ({
    ...product,
