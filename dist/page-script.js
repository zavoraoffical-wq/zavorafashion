...
const pageMegaMenuData = {
  ...
  men: {
    label: '',
    title: 'Heavyweight layers, oversized fits, sweatpants, jackets, shorts, and accessories.',
    href: 'men.html',
    image: 'assets/zavora-men-cutout.png',
    items: [['Oversized Tees', 'oversized-tees'], ['Heavyweight Tees', 'heavyweight-tees'], ['Hoodies', 'hoodies'], ['Sweatpants', 'sweatpants'], ['Jackets', 'jackets'], ['Shorts', 'shorts'], ['Accessories', 'accessories']]
  }
};
...
function productsForCatalogPage(products, pageName) {
  ...
  if (urlCategory) {
    const filtered = products.filter((product) => {
      const targetPages = Array.isArray(product.targetPages) ? product.targetPages.map((item) => String(item).toLowerCase()) : [];
      return categoryMatches(product.category, urlCategory) || 
             targetPages.includes(`category:${String(urlCategory).toLowerCase()}`) ||
             String(product.name || '').toLowerCase().includes(urlCategory.replace(/-/g, ' '));
    });
    if (filtered.length) {
      products = filtered;
    }
  ...
  if (norm === 'new-arrivals') {
    const newItems = products.filter((product) => product.collection?.includes('new'));
    return newItems.length ? newItems : products;
  }
  if (norm === 'limited') {
    const limItems = products.filter((product) => product.collection?.includes('limited'));
    return (limItems.length ? limItems : products)
      .map((product, index) => ({ ...product, stock: Math.min(getProductStock(product), 1 + (index % 10)), badge: 'Limited' }));
  }
  if (norm === 'best-sellers') {
    const bestItems = products.filter((product) => product.collection?.includes('best') || product.popularity >= 84);
    return bestItems.length ? bestItems : products;
  }
  return products.length ? products : initialPool;
}
...
function injectLargeCatalog() {
  ...
  const categoryOptions = isWomenPage
    ...
    : '<option value="all">All</option><option value="oversized-tees">Oversized Tees</option><option value="heavyweight-tees">Heavyweight Tees</option><option value="hoodies">Hoodies</option><option value="sweatpants">Sweatpants</option><option value="jackets">Jackets</option><option value="shorts">Shorts</option><option value="accessories">Accessories</option>';
...
function initMobileFilterDrawer(genderOptions, collectionOptions, categoryOptions, pageName) {
  ...
  drawer.innerHTML = `
    ...
    <div class="mobile-filter-body">
      ${pageName === 'shop' || pageName === 'collections' ? `
