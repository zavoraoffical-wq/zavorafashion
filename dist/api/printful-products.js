...
function isWomenCatalogProduct(product) {
  const text = `${product?.name || ''} ${product?.external_name || ''} ${product?.sync_product?.name || ''} ${product?.title || ''} ${product?.type_name || ''} ${product?.description || ''}`.toLowerCase();
  if (blockedStorefrontProductTerms.test(text)) return false;
  const isShirtLike = /(tee|t-shirt|shirt|polo|tank top|crop top|cropped top|baby tee)/i.test(text);
  const allowed = isShirtLike || /(women|women's|ladies|female|crop|cropped|baby tee|hoodie|zip|quarter-zip|sweatshirt|pullover|fleece|sweatpants|jogger|sport|performance|athletic|training|gym|active|set|matching|tracksuit|beach|slide)/i.test(text);
  const blocked = /(shorts?|sweatpants?|joggers?|pants|legging|dress|skirt|underwear|boxer|brief|trunk|thong|panties|bra|bikini|sock|backpack|bag|tote|duffle|luggage|tag|headband|neck gaiter|rash guard|kids|youth|baby clothes|toddler|rug|ornament|poster|mug|canvas|sticker|phone|pillow|blanket|towel|apron|pet|case|sleeve|laptop|bottle|mouse pad|notebook|journal|stationery|tumbler|cup|mug|straw|drinkware|water bottle|card|postcard|poster)/i.test(text);
  return allowed && !blocked && !product?.is_discontinued;
}
