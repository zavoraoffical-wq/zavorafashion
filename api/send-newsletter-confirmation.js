...
function senderFromEnv() {
  const configured = process.env.NEWSLETTER_FROM_EMAIL || process.env.WELCOME_EMAIL || 'Zavora Fashion <hello@zavorafashion.com>';
...
module.exports = async function handler(req, res) {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;background:#f5f5f5;padding:28px;color:#111">
      <div style="max-width:620px;margin:0 auto;background:#fff;border:1px solid #ddd;padding:30px">
        ...
        <h1 style="font-family:Georgia,serif;font-size:38px;line-height:1.05;margin:18px 0">You're on the list.</h1>
        <p>Welcome to the Zavora Fashion newsletter.</p>
        <p>You will receive new drops, private edits, style notes, and early access to limited releases.</p>
  ...
  const response = await fetch(RESEND_API_URL, {
    ...
    body: JSON.stringify({
      from: senderFromEnv(),
      to,
      subject: 'Welcome to the Zavora Fashion newsletter',
      html,
      text: 'Welcome to the Zavora Fashion newsletter. You will receive new drops, private edits, style notes, and early access to limited releases.'
    })
  });

  const data = await response.json().catch(() => ({}));
  if (!response.ok) return json(res, response.status, { error: data.message || 'Could not send newsletter email' });
